...
def build_generic_spec(first_line_indent: bool = True) -> StyleSpec:
    ...
    # 参考文献 - 五号宋体，悬挂缩进约1.5字符（≈17pt/10.5pt），1.5倍行距
    add_style(
        "Reference", "参考文献",
        song, times, pt("五号"),
        bold=False, align="justify",
    ...
    # 目录一级条目 - 四号宋体，1.5倍行距（映射到 Word TOC 1）
    add_style(
        "Toc1", "目录一级",
        song, times, pt("四号"),
        bold=False, align="left",
    ...
    # 目录二级条目 - 小四宋体，1.5倍行距，左缩进21pt（映射到 Word TOC 2）
    add_style(
        "Toc2", "目录二级",
        song, times, pt("小四"),
        bold=False, align="left",
    ...
    # 目录三级条目 - 小四宋体，1.5倍行距，左缩进42pt（映射到 Word TOC 3）
    add_style(
        "Toc3", "目录三级",
        song, times, pt("小四"),
        bold=False, align="left",
...
AI_SPEC_GENERATION_PROMPT = """你是一个专业的中国学术论文排版专家。请根据以下输入生成论文排版规范模板。

【输入数据说明】
输入可能包含以下几类数据，请按优先级使用：
1. [检测到的段落样式]（高优先级）：从 Word 文件中直接读出的字体/字号/对齐等，比文字描述更准确
2. [批注内容]（最高优先级）：文档作者/教师添加的格式说明，通常是最直接的格式要求
3. [文档正文]：文字内容，辅助理解语境

优先级顺序：批注内容 > 检测到的样式 > 文字描述
...
{{
    ...
    "header": {{
        "enabled": true或false（文档是否要求页眉）,
        "content_type": "blank|school_name|thesis_title|chapter_title|custom",
        "custom_text": null或自定义文字（content_type=custom时填写）,
        "font_name": "SimSun",
        "font_size_pt": 9,
        "alignment": "center|left|right",
        "bold": false,
        "separator_line": true（是否显示页眉下分隔线）
    }},
    "footer_content": {{
        "enabled": false（页脚内容，通常为空，页码由page_numbering控制）,
        "content_type": "blank",
        "font_name": "SimSun",
        "font_size_pt": 9,
        "alignment": "center",
    ...
    "cross_ref": {{
        ...
        "table_prefix": "表",
        "equation_prefix": "式",
        "numbering_style": "chapter-seq|global-seq",
        "separator": "-",
        "caption_position_figure": "below|above",
        "caption_position_table": "above|below",
...
【页眉/页脚提取规则】
- 若文档提到"页眉写学校名称/论文题目/章节标题"，设 enabled:true，content_type 对应填写
- 若未提到页眉，设 enabled:false，content_type:"blank"
- 页眉字体字号若未说明，默认小五号宋体(9pt, SimSun)居中

【交叉引用提取规则】
- 若文档提到"图×-×"格式（含章号），numbering_style:"chapter-seq"；若只说"图×"，则"global-seq"
- 图注一般在图下方(below)，表注一般在表上方(above)
- 若文档未提及，保留上述默认值

【目录条目样式说明】
若文档有目录，必须包含 Toc1/Toc2/Toc3 样式（系统自动映射到 Word 内置 TOC 1/2/3）：
- Toc1: style_id="Toc1", name="目录一级"，一般为四号14pt，1.5倍行距，left_indent_pt=0
- Toc2: style_id="Toc2", name="目录二级"，一般为小四12pt，1.5倍行距，left_indent_pt=21
- Toc3: style_id="Toc3", name="目录三级"，一般为小四12pt，1.5倍行距，left_indent_pt=42
paragraph 字段中设置 "left_indent_pt": <数值> 实现左缩进

【参考文献悬挂缩进说明】
参考文献样式使用 hanging_indent_chars 设置悬挂缩进：
- hanging_indent_chars=1.5 表示左缩进约15.75pt（≈17pt），首行缩进-15.75pt
- 不要同时设置 first_line_indent_chars 和 hanging_indent_chars
- 常见格式: 五号宋体(10.5pt)，1.5倍行距，hanging_indent_chars=1.5

【致谢/附录标题样式说明】
- AcknowledgementTitle: 致谢标题，三号16pt，黑体，居中，加粗，段前24pt，段后18pt
- AppendixTitle: 附录标题，格式同 AcknowledgementTitle

【重要提示】
1. 只返回 JSON，不要任何其他文字
2. 确保 JSON 格式正确，所有字符串用双引号
3. 数值不要加引号
4. 必须包含所有必要的样式: TitleCN, Body, H1, H2, H3, AbstractBody, KeywordsBody, Reference, Toc1, Toc2, Toc3, AcknowledgementTitle, AppendixTitle
5. header、footer_content、cross_ref 字段必须包含（不可省略）
"""
